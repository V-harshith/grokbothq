...
- After any merge that changed content, ping the revalidation gate once so pages
  refresh instantly (belt and suspenders alongside the auto-deploy):
  `curl "https://grokbothq.xyz/api/revalidate?secret=$REVALIDATE_TOKEN"`
...
- **Revalidation gate**: `GET /api/revalidate?secret=$REVALIDATE_TOKEN` regenerates all pages
  instantly after content changes - use it after every merge.
